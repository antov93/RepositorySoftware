...
public class Classe extends Frame {
	...
	private JScrollPane scrollPaneClasse;
	private JTable classe;
	
	public Classe(){
		super();
		inizializzaComponentiGrafiche();
		setFrameTable(classe);
	...
	private void inizializzaComponentiGrafiche(){
		...
		scrollPaneClasse= new JScrollPane();
		classe = new JTable();
        ...
        scrollPaneClasse.setViewportView(classe);
        add(scrollPaneClasse);
        scrollPaneClasse.setEnabled(true);
	...
	protected PreparedStatement getComandoInserimento(Connection conn) throws SQLException {
		  String comando;
	      PreparedStatement ps;
	      comando = "INSERT INTO CLASSE (idClasse,nome,descrizione,aggiunto,modificato,eliminato,package) "
	      		+ "VALUES(?,?,?,?,?,?,?)";
	      ...
	      ps.setString(7, package_.getText());
	      return ps;
	...
	protected PreparedStatement getComandoModifica(Connection conn) throws SQLException {
	      ...
	      if(colonna==6) ps.setString(1, package_.getText());
