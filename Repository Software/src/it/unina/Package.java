...
public class Package extends Frame {
	...
	private JTable package_;

	public Package(){
		super();
		inizializzaComponentiGrafiche();
		setFrameTable(package_);
		setNomeTabella("PACKAGE");
		visualizzaTabella();
	}
...
private void inizializzaComponentiGrafiche(){
        ...
        scrollPanePackage.setViewportView(package_);
        add(scrollPanePackage);
        scrollPanePackage.setEnabled(true);
        package_.setEnabled(true);
...
private void visualizzaTabella(){
	...
	protected PreparedStatement getComandoInserimento(Connection conn) throws SQLException {
		  String comando;
	     PreparedStatement ps;
	     comando = "INSERT INTO PACKAGE (idPackage,nome,descrizione,aggiunto,modificato,eliminato,release) "
	     		+ "VALUES(?,?,?,?,?,?,?)";
	...
	protected PreparedStatement getComandoModifica(Connection conn) throws SQLException {
	      ...
	      if(colonna==6) ps.setString(1, release.getText());
